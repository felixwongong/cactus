...
Both INT4 and INT8 precision are tested. Most backends use group-wise quantization with a group size of 32, where each group of 32 weight elements along the K dimension shares a single scale factor. **Note:** LiteRT's INT4 backend (`litert_4bit_neon`) uses per-channel quantization (one scale per output row) rather than per-group, which reduces compute per element and gives it an inherent speed advantage at the cost of quantization quality.
...
Before timing, each backend runs once with output capture enabled. The captured fp32 output is compared against a naive fp64-accumulated reference matmul using Normalized Root Mean Square Error (NRMSE). INT8 backends must achieve NRMSE < 0.05; INT4 backends must achieve NRMSE < 0.20. Backends that use their own internal quantization (GGML, LiteRT) provide a dequantized reference for self-consistency checking with a tighter 0.01 tolerance.
...
NRMSE values of 0.0000 for GGML and LiteRT indicate self-consistency checks (they re-quantize internally and compare against their own dequantized reference), not higher accuracy. All other backends are compared against a naive fp64 reference matmul.
