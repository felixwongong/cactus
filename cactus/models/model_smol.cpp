...
SmolModel::SmolModel() : Model() {}

SmolModel::SmolModel(const Config& config) : Model(config) {
    weight_nodes_.layers.resize(config.num_layers);
}

void SmolModel::load_weights_to_graph(CactusGraph* gb) {
...
size_t SmolModel::SmolModel::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    if (use_cache && !kv_cache_.is_empty()) {
        attn_output_4d = gb->attention_int8_hybrid(
            ...
            kv_cache_.current_seq_len, config_.attention_kv_heads, config_.attention_head_dim
        );
    } else {
...
size_t SmolModel::build_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx,
